...
export default function retainHocReducer(reducers) {
  ...
  return (state = {}, action) => {
    ...
    if (url && page && Object.prototype.hasOwnProperty.call(shouldRetainReducers, page)) {
      const currentStateForUrl = state[url];
      const fn = shouldRetainReducers[page];
